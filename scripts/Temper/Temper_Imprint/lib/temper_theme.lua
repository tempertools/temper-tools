-- ============================================================
-- temper_theme.lua -- Temper shared GUI theme (Spectral Core)
-- ============================================================
-- Usage: dofile(reaper.GetResourcePath() .. "/Scripts/Temper/lib/temper_theme.lua")
--   temper_theme.attach_fonts(ctx)   -- call once after CreateContext, before first Begin
--   local n = temper_theme.push(ctx) -- call at top of each defer frame
--   temper_theme.pop(ctx, n)         -- call at bottom of each defer frame
--
-- This module sets the global `temper_theme` table so dofile()-based loading works.
-- ============================================================

-- Spectral Core full palette -- exported as temper_theme.SC for component-level use.
...
-- After this call, temper_theme.font_regular, font_bold, and font_hero are valid.
...
-- Expose as global `temper_theme` so dofile()-based loading works.
temper_theme = {
  ...
  -- Usage: if temper_theme.font_regular then reaper.ImGui_PushFont(ctx, temper_theme.font_regular, 13) end
  -- Usage: if temper_theme.font_bold then reaper.ImGui_PushFont(ctx, temper_theme.font_bold, 13) end
  -- Usage: if temper_theme.font_hero then reaper.ImGui_PushFont(ctx, temper_theme.font_hero, 18) end
