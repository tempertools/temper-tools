...
--
-- Public API:
--   rsg_license.check(product_code, ctx)    -> "licensed" | "trial" | "expired"
--   rsg_license.configure(opts)             -> set namespace, scope_id, buy_url, display_name
--   rsg_license.open_dialog(ctx)            -> call once inside Begin/End to show dialog
--   rsg_license.draw_dialog(ctx)            -> call each frame while is_dialog_open() is true
--   rsg_license.is_dialog_open()            -> bool
--   rsg_license.trial_days_remaining()      -> int (0 when licensed or expired)
--
-- Dependencies (loaded lazily at first check()):
--   lib/rsg_sha256.lua    -- pure Lua SHA-256 + HMAC-SHA256
--   lib/rsg_activation_dialog.lua -- visual activation dialog
--   lib/rsg_theme.lua     -- required by rsg_activation_dialog
--
...
-- Works whether rsg_license.lua lives in the dev shared lib/ or in a per-package
...
local function _require_sha()
  ...
  local ok, mod = pcall(dofile, _lib_path("rsg_sha256.lua"))
  if not ok then
    error("[rsg_license] Cannot load rsg_sha256.lua: " .. tostring(mod), 2)
  end
  _sha = mod
  return _sha
end

-- ctx is passed so that if rsg_theme must be loaded here as a fallback,
-- attach_fonts(ctx) can be called to satisfy rsg_activation_dialog's font_bold assertion.
local function _require_dialog(ctx)
  if _dlg then return _dlg end
  -- rsg_activation_dialog requires rsg_theme to be a global.
  if type(rsg_theme) ~= "table" then
    local theme_ok, theme_err = pcall(dofile, _lib_path("rsg_theme.lua"))
    if not theme_ok then
      error("[rsg_license] Cannot load rsg_theme.lua: " .. tostring(theme_err), 2)
    end
  end
  -- rsg_activation_dialog asserts rsg_theme.font_bold ~= nil at load time.
  -- If fonts were not attached at startup (e.g. rsg_theme.lua was missing then),
  -- attach them now using the ctx we received.
  if type(rsg_theme) == "table" and rsg_theme.font_bold == nil and ctx then
    rsg_theme.attach_fonts(ctx)
  end
  local ok, mod = pcall(dofile, _lib_path("rsg_activation_dialog.lua"))
  if not ok then
    error("[rsg_license] Cannot load rsg_activation_dialog.lua: " .. tostring(mod), 2)
  end
  _dlg = mod
