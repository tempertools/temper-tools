...
-- ============================================================
-- Exported: Onset detection from a pre-built envelope
-- ============================================================

--- Run the flux-based onset pipeline against an already-computed envelope.
-- Unlike detect_markers() which streams a WAV file, this takes a plain
-- chronological array of per-hop energy/peak values. Used by real-time
-- ring-buffer callers (e.g., Temper Recall) where the "envelope" is a
-- peak-per-slot ring decoded from gmem each frame.
--
-- env:     array (1-based) of non-negative scalars, env[1] = oldest
-- hop_sec: seconds covered by one env slot (e.g., buf_dur / DISP_RES)
-- params:  { silence_db, sensitivity, spacing_ms }
-- Returns: array of env indices (1-based, chronological) where onsets fire.
function M.detect_onsets_from_envelope(env, hop_sec, params)
  local n = env and #env or 0
  if n < 8 then return {} end

  local flux = _build_flux(env)
  local peaks = _find_flux_peaks(flux, 4)
  if #peaks == 0 then return {} end

  peaks = _apply_prominence_filter(peaks, flux, env, params, 0)
  if #peaks == 0 then return {} end
  ...
  local onsets = {}
  for _, idx in ipairs(peaks) do
    onsets[#onsets + 1] = _backtrack_to_onset(idx, env, silence_lin, 15)
  end

  return _enforce_spacing(onsets, min_hops)
end
