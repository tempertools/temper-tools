...
--
--   Integration:
--     The FE wires this into rsg_license.lua's draw_dialog() function.
--     This file is the DE's complete visual handoff — do not add business logic here.
--
--   Depends on:
--     rsg_theme  — must be dofile'd before this module so rsg_theme is a global.
--                  rsg_theme.apply(ctx) and rsg_theme.attach_fonts(ctx) must be
--                  called before the first frame. If rsg_theme is missing, an error
--                  is raised at load time with an actionable message.
--
--   Public API:
--     rsg_activation_dialog.open(ctx, trial_days)
--       Call once when you want to show the dialog. Idempotent.
--       trial_days: integer (days remaining), or 0 if expired.
--
--     rsg_activation_dialog.draw(ctx, trial_days) → string|nil
...
--
--     rsg_activation_dialog.last_key  → string  (raw input on activate_requested)
--     rsg_activation_dialog.set_error(msg)       (call when FE rejects the key)

-- ── Guard: require rsg_theme to be loaded before this module ─────────────────
assert(type(rsg_theme) == "table" and rsg_theme.font_bold ~= nil,
  "[rsg_activation_dialog] rsg_theme not loaded. " ..
  "Call dofile(rsg_theme_path) and rsg_theme.attach_fonts(ctx) before using this module.")

-- ── Color aliases — source from rsg_theme tokens ─────────────────────────────
local C_TEAL = rsg_theme.TEXT_TEAL  -- active/positive status
local C_RED  = rsg_theme.TEXT_RED   -- error / expired
...
function M.draw(ctx, trial_days)
  ...
  rsg_theme.push_neutral_btn(ctx)
